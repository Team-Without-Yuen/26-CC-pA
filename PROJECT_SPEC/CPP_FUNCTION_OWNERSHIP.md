...
| `src/analysis/FunctionAnalysis.cpp` | Boolean Function & Search Analysis | CaDiCaL SAT helper、equivalence、dependence、symmetry/cofactor miter、constant/truth analysis、共用 bit-parallel simulator、simulation-filtered NAND pair search、`runFunctionQuery()`、`runFunctionSearchQuery()` |
| `src/analysis/SequentialPatternAnalysis.cpp` | Sequential Pattern Facade | DFF target dispatch、canonical fast path、generic match 到 sequential report 的轉換、aggregate completeness、`runSequentialPatternQuery()` |
| `src/analysis/FunctionalPatternEngine.cpp` | Technology-independent Functional Pattern Engine | matcher registry、PI/leaf/D-input distance 與 simulation-aware candidate ranking、candidate limit、共用 simulation prefilter、control reachability、one-shot/reusable hybrid SAT cofactor proof；目前實作 MUX-hold matcher |
...
| `src/core/MockturtleConverter.cpp` | Mockturtle Adapter | Netlist 與 AIG/XAG 往返轉換、PI/PO/DFF boundary 保留、轉換後重複 inverter cleanup |
...
| Sequential pattern query/report | `src/analysis/SequentialPatternAnalysis.cpp` + `FunctionalPatternEngine.cpp` | canonical 與 opt-in functional MUX-hold 已完成；AND-only semantics 待官方確認 |
...
4. Mockturtle / ABC 已由 adapter 與 optimization core 分離；後續不要把第三方 network 操作塞回 core Netlist。
```
