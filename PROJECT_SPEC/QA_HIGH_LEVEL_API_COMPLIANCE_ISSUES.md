...
| QAC-006 | 缺 all register-to-register path wrapper | `PathQuery`, `DepthQuery` | register-to-register path 可由 DFF.Q 到 DFF.D 表示 | 現在可查單一 `DffQ -> DffD`，但沒有全域 wrapper | 問所有 reg-to-reg paths / longest reg-to-reg path 時需要外層 loop | P1 | 新增 batch wrapper：all DFF.Q to all DFF.D 的 exists / max depth / enumerate summary |
