...
完整性規則：all-DFF records 必須取完 pagination；functional candidate/match/data caps 不得被
當成完整 all-result。時間限制依題目指定。詳見 [`LLM_NOTES.md`](LLM_NOTES.md)。
...
- detail pagination 的 envelope 會在尚有下一頁時回 `partial`；要求完整名單時必須依
  `next_record_offset` 取完所有頁面，不得只交第一頁。
